...
export function buildQueryRecursively<T>(
    ...
    const selectedFields = tree.fields
        .filter((field: GraphQLQueryTree<T>) => metadata.propertiesMap[field.name] && !field.isRelationIn(metadata))
    ...
    // Fourthly, we check the special selectAlways decorator data and force select those columns
